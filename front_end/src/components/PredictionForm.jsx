...
import { DEFAULT_FORM_VALUES, WORKOUT_TYPES, DIET_TYPES, EXPERIENCE_LEVELS } from '../utils/constants';
...
const PredictionForm = ({ onSubmit, loading }) => {
  ...
  const calorieBalance = calories && caloriesBurned ? calories - caloriesBurned : null;
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            />
            {errors.max_bpm && (
              <p className="text-red-500 text-xs mt-1">{errors.max_bpm.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            />
            {errors.avg_bpm && (
              <p className="text-red-500 text-xs mt-1">{errors.avg_bpm.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            />
            {errors.resting_bpm && (
              <p className="text-red-500 text-xs mt-1">{errors.resting_bpm.message}</p>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            {errors.calories_burned && (
              <p className="text-red-500 text-xs mt-1">{errors.calories_burned.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            {errors.workout_frequency && (
              <p className="text-red-500 text-xs mt-1">{errors.workout_frequency.message}</p>
            )}
          </div>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            />
            {errors.proteins && (
              <p className="text-red-500 text-xs mt-1">{errors.proteins.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            />
            {errors.sugar_g && (
              <p className="text-red-500 text-xs mt-1">{errors.sugar_g.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            {errors.daily_meals_frequency && (
              <p className="text-red-500 text-xs mt-1">{errors.daily_meals_frequency.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            />
            {errors.water_intake && (
              <p className="text-red-500 text-xs mt-1">{errors.water_intake.message}</p>
            )}
          </div>
        </div>
        ...
        {calorieBalance !== null && (
          <div className="mt-6 p-4 rounded-lg bg-gradient-to-r from-blue-50 to-gray-50 border">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="font-semibold text-gray-900">Calorie Analysis</h4>
                <p className={`text-sm font-medium ${
                  calorieBalance > 0 ? 'text-red-600' : 
                  calorieBalance < -300 ? 'text-green-600' : 
                  'text-yellow-600'
                }`}>
                  {calorieBalance > 0 ? 'Surplus' : 'Deficit'}: {Math.abs(calorieBalance).toFixed(0)} calories
                </p>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-500">Intake: {calories} cal</p>
                <p className="text-xs text-gray-500">Burned: {caloriesBurned} cal</p>
              </div>
            </div>
            <div className="mt-2 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div 
                className={`h-full rounded-full ${
                  calorieBalance > 0 ? 'bg-red-500' : 
                  calorieBalance < -300 ? 'bg-green-500' : 
                  'bg-yellow-500'
                }`}
                style={{ 
      ...
      <div className="sticky bottom-6 z-10">
        <div className="bg-white rounded-xl shadow-2xl border border-gray-200 p-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
              <h4 className="text-lg font-bold text-gray-900">Ready for Analysis?</h4>
